...
char** parse_input_string(char* raw_string, int str_len, int* num_of_parsed_symbols_out) {
	...
	char** symbols = (char**)malloc(sizeof(char*) * MAX_SYMBOL_SEQUENCE_LENGTH);// array of separated symbols
	int symbol_pointer = 0;

	char* buffer = (char*)malloc(MAX_SINGLE_TOKEN_LENGTH); // contains current string 
	...
	for (int i = 1; i <= str_len; i++) {
		if ((syntax_map[i] == m_OPERATOR && syntax_map[i - 1] == m_OPERATOR) 
			...
			symbols[symbol_pointer] = (char*)malloc(buffer_pointer);
 ...
 token* lexer(char** symbols, int symbol_num) {
	token* token_array = (token*)malloc(sizeof( token) * symbol_num);
...
void paste_symbol(char** dst, int symb_ptr, int* arr_ptr, char* src , char symb) {
	dst[*arr_ptr] = (char*)malloc(symb_ptr);
	memcpy(dst[*arr_ptr], src, symb_ptr);
	dst[*arr_ptr][symb_ptr] = '\0';
	++(*arr_ptr);

	dst[*arr_ptr] = (char*)malloc(2);
	dst[*arr_ptr][0] = symb;
	dst[*arr_ptr][1] = '\0';
	++(*arr_ptr);
