...
float solve(node* root) {
	...
	find_x(root);
	res = root->m_child_right->cnt.m_number;
...
int calculate_node(node* _node){
	float l_num = _node->m_child_left->cnt.m_number;
	float r_num = _node->m_child_right->cnt.m_number;
	float res = 0;
...
void find_x(node* _node) {
	...
	while ((*left_branch_ptr)->m_type != n_VARIABLE) {
		...
		operate_node((*left_branch_ptr)->cnt.m_operator, *right_branch_ptr, (*left_branch_ptr)->m_child_right);
...
void operate_node(enum operators op, node* right_node, node* num_node){
	...
	{
	case op_SUBSTRACT:
		right_node->cnt.m_number += num_node->cnt.m_number;
		break;
	case op_ADD:
		right_node->cnt.m_number -= num_node->cnt.m_number;
		break;
...
void solve_node_with_variables(node* _node) {
	if (_node->m_type == n_OPERATOR && (_node->cnt.m_operator == op_MULIPLY || _node->cnt.m_operator == op_DEVIDE)) {
