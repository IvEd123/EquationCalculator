...
static void configure_node(node* _node, char symbol[], token token) {
	...
	case t_NUMBER:
		...
		_node->cnt.m_number = (float)atof(symbol);
		break;
	case t_ADDITION:
		_node->m_type = n_OPERATOR;
		_node->cnt.m_operator = op_ADD;
		break;
	case t_MULIPLICATION:
		_node->m_type = n_OPERATOR;
		_node->cnt.m_operator = op_MULIPLY;
		break;
	case t_SUBSTRACTION:
		_node->m_type = n_OPERATOR;
		_node->cnt.m_operator = op_SUBSTRACT;
		break;
	case t_DIVISION:
		_node->m_type = n_OPERATOR;
		_node->cnt.m_operator = op_DEVIDE;
		break;
	case t_EQUALS:
		_node->m_type = n_OPERATOR;
		_node->cnt.m_operator = op_EQUALS;
		break;
	default:
		break;
	}
...
node* build_tree(token* token_array, char** symbols, int length) {
	...
	int level = 0;
	int* mult_ops_array = (int*)malloc(length);
	int mult_ops_num = 0;
...
static bool has_variables(node* _node) {
	...
	if (_node->m_type == n_OPERATOR) {
		...
		res = left || right;
		if (left && right && (_node->cnt.m_operator == op_MULIPLY || _node->cnt.m_operator == op_DEVIDE)) {
